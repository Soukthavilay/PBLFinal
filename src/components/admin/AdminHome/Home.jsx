...
const Home = () => {
  ...
  const fetchStatistics = async (year, month) => {
    try {
      let res;
      if (!year || !month) {
        res = await axios.get("https://pbl-technology-988327da4050.herokuapp.com/api/report");
      } else {
        res = await axios.get(`https://pbl-technology-988327da4050.herokuapp.com/api/report/${year}/${month}`);
      }
